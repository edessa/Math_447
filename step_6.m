...
rgb_image = double(imread('doge.png'));
...
for x = 1: floor(size(rgb_image,1)/8)
	for y = 1: floor(size(rgb_image,2)/8)
		...
		[compressed_Y, transform1] = dct(double(Y_block));
		[compressed_U, transform2] = dct(double(U_block));
		[compressed_V, transform3] = dct(double(V_block));
...
combined_compressed = cat(3, red_comp, blue_comp, green_comp);
combined_decompressed = cat(3, blue_decomp, red_decomp, green_decomp);

imagesc(uint8(combined_compressed))
imagesc(uint8(combined_decompressed))
